...
const useLaboratory = (props) => {
  ...
  const [instanceContractLaboratory, setInstanceContractLaboratory] = useState(false)
  ...
  const getActiveContract = async () => {
    try {
      const response = await instanceContract.createLaboratoryFactory().send({ from: account[0] });
  ...
  const getBalanceLaboratory = async () => {
    try {
      let response = await instanceContractLaboratory.balanceLaboratory().call();
  ...
  const cancelContract = async () => {
    try {
      await instanceContractLaboratory.cancelContractLaboratory().send({ from: account[0] });
  ...
  const withdrawalMoney = async (quantity) => {
    try {
      ...
      await instanceContractLaboratory.withdrawBalanceLaboratory(quantity).send({ from: account[0] });
  ...
  const onChangeStatusSpecialService = async (name) => {
    try {
      await instanceContractLaboratory.changeStatusServiceLaboratory(name).send({ from: account[0] });
      getSpecialServices();
  ...
  const getCreateSpecialService = async (price, name) => {
    try {
      await instanceContractLaboratory.createSpecialService(name, price).send({ from: account[0] });
      getSpecialServices()
